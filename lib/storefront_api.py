...
class store_info():
    ...
    # returns applist dataframe and a list apps_lst for scraping
        if self.apps == 0 and use_old_db == False:
            try:
                games = requests.get('https://api.steampowered.com/IStoreService/GetAppList/v1/?include_games=1&include_dlc=0&include_software=0&include_videos=0&include_hardware=0&key={0}&max_results=50000'.format(self.key))
    ...
    def get_store_info(self, apps, key = None, last_df = None, start_time = time.time(), rebuild = False, test = False):
        ...
        with open('data.json', 'a', encoding='utf-8') as f:
            ...
            for i, app in enumerate(apps["appid"]):
                ...
                keyid = str(app)
                appresp = requests.get('https://store.steampowered.com/api/appdetails?appids={0}&key={1}'.format(app,self.key)).json()
                if i != 0 and i % 198 == 0:
                    end_time = time.time()
                    elapsed = end_time-batch_time
                    print("Batch {} of {} finished in {:.2f} seconds. Sleeping for 5 minutes.".format(math.ceil(i/198), batches_total, elapsed))
                    batch_time = time.time()
                    time.sleep(300) #Even though there's no documentation on it, steam API seems to lock after 199 calls, hence the batching. It's supposedly 200 every 5 minutes, but I was consistently 429'd at 199.    
                elif i == limit-1:
                    end_time = time.time()
